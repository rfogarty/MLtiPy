...
class SnapshotEnsemble(Callback):
    ...
    def cosine_annealing(self, iteration, lrate_max):
        if self.warmupEpochs > 0 :
            ...
            self.cos_inner = (math.pi * (iteration % self.iterationsPerCycle)) / (self.iterationsPerCycle)
            alpha = 0.02 # Value dips to 2% of the original value.
            scale = (math.cos(self.cos_inner) + 1.0)/2.0
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if self.warmupEpochs > 0 :
            self.warmupEpochs -= 1
            if self.verbose:
                print(f'> (warming up) no snapshot saved at epoch {epoch}')
        # check if we can save model
            ...
            print(f'>saved snapshot {filename}, on epoch/iteration/cos_inner {epoch}/{self.iteration}/{self.cos_inner}')
            self.iteration = 0
        elif self.verbose:
            print(f'>no snapshot saved at epoch/iteration/cos_inner {epoch}/{self.iteration}/{self.cos_inner}')
